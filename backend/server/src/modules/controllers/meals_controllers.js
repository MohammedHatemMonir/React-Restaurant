...
const updateMeal = async (req, res) => {
  try {
    ...
    global.io.to(restaurant.ownerId?.toString()).emit("new-notification", {message: `Meal updated`, time: Date.now(), link: "/" });
...
const deleteMeal = async (req, res) => {
  try {
    ...
    global.io.to(restaurant.ownerId?.toString()).emit("new-notification", {message: `Meal deleted`, time: Date.now(), link: "/" });
...
const createOrder = async (req, res) => {   // {ResId:"",meals:[{id:"",quantity:""}]}
  try {
    ...
    global.io.to(restaurant.ownerId?.toString()).emit("new-notification", {message: `Someone created an order`, time: Date.now(), link: "/" });
    global.io.to(req.session?.user?._id).emit("new-notification", {message: `Your order has been successfully created`, time: Date.now(), link: "/" });
