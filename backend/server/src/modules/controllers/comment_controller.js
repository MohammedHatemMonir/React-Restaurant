  ...
  try {
    ...
    // }else{
        ...
        if (!resturants[0]) {
          ...
          global.io.to(resturants[0].ownerId?.toString()).emit("new-notification", {message: "Your restaurant has recieved a new Rating.", time: Date.now(), link: `/restaurant/${resturants[0]._id}/${resturants[0].ResName}` });
          global.io.to(req.session.user._id).emit("new-notification", {message: `You have placed a new rating at ${resturants[0].ResName}`, time: Date.now(), link: `/restaurant/${resturants[0]._id}/${resturants[0].ResName}` });
