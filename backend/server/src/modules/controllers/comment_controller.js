  ...
  try {
    ...
    // }else{
        ...
        if (!resturants[0]) {
          ...
          global.io.to(resturants[0].ownerId).emit("new-notification", {message: "Your restaurant has recieved a new Rating.", time: Date.now().toString() });
          global.io.to(req.session.user._id).emit("new-notification", {message: `You have placed a new rating at ${resturants[0].ResName}`, time: Date.now().toString() });
