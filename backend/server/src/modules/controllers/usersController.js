...
const TestNotifs = async (req, res) => {
  try {
    ...
    global.io.to("User Or Room ID here").emit("new-notification", { message: "You have recieved a new Rating.", time: Date.now() });
  ...
  try {
    ...
    if (userImg) {
      try {
        ...
        return res.status(500).json({ success: false, message: 'An error occurred while uploading the image.', error: error.message });
      }
    }
