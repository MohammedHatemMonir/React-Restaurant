...
const genAI = new GoogleGenerativeAI("AIzaSyBxT0_d7U5DQHbqofsQhkUqv5nMDI-4Xlw");
  ...
  try {
    ...
    if (containedWord) {
      ...
      if (containedWord === "Orders:") {
        ...
        const CategoryToSearch = aiText
          .trim()
          .replace(/Suggest: /g, "")
          .replace(/\.$/, "")
          .replace(/^\s+/, "");
        const category = await Category.aggregate([{ $match: { category: { $regex: CategoryToSearch, $options: "i" } } }, { $limit: 1 }]);
        if (category[0]) {
        ...
        const FindRestaurant = await restaurant.aggregate([{ $match: { ResName: { $regex: ResToSearch, $options: "i" } } }, { $limit: 1 }]);
        if (FindRestaurant[0]) {
          CmdChatText = `System prompt: Found the restaurant ${FindRestaurant[0].ResName},With a rating of ${
            FindRestaurant[0].rating * 5
          } stars.Explain the restaurant breifly and Tell the user that we have that restaurant and you'll navigate them soon. Also mention that they can leave a review.`;
          navigationLink = `/restaurant/${FindRestaurant[0]._id}/${FindRestaurant[0].ResName}`;
        }
        // }
        // if(containedWord === "Meal:"){
        if (CmdChatText === null) {
          const mealToSearch = aiText
            .trim()
            .replace(/Meal: /g, "")
            .replace(/\.$/, "")
            .trim();
          ...
          if (FindMeal[0]) {
            CmdChatText = `System prompt: Found the Meal ${FindMeal[0].MealName}, It's description is ${FindMeal[0].Description}, with a rating of ${FindMeal[0].rating} stars.Explain the meal breifly and Tell the user that we have that Meal and you'll navigate them soon. Also mention that they can leave a review.`;
      ...
      const response = await result.response;
      req.session.history.push({ role: "user", parts: [{ text: CmdChatText }] });
